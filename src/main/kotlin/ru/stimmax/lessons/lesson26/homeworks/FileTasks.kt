...
Запишите в него строку "Hello, Kotlin!",
а затем проверьте, существует ли файл.
...
Проверьте, является ли она директорией,
...
а так же метод file.resolve("myDir") для создания нового объекта File
путём добавления к существующему пути ещё одной секции.
...
fun ex3() {
    File("workspace/task3/structure").apply {
        ...
        resolve("myDir").apply {
            ...
            val subDirs = listFiles()?.map { it.name } ?: emptyList()

            if (subDirs.containsAll(listOf("subDir1", "subDir2"))) {
                println("Директории найдены")
            } else {
                println("Директории не найдены: $subDirs")
            }
        }
    }
}
...
и скопируйте туда файлы из workspace/task8/data
сохраняя структуру директорий.

Для копирования используй метод copyTo.
Для получения относительного пути начиная от data
используй relativeTo.
*/
fun ex8() {
